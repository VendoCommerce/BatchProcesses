...
namespace PetaDolexEmailDirect
{
    class Batch
    {
        ...
        void UpdateEmailDirectStatus_SubscribeEmailList(string Created, string emailAddress)
        {
            try
            {
                string sqlquery = "update [dbo].[signups] set EmailDirectStatus=1 where CAST(CONVERT(VARCHAR(20),Date,113) AS DATETIME) = CONVERT(VARCHAR(20),CAST('" + Created + "' AS DATETIME),113) and [Email]  ='" + emailAddress + "'";
                runsql(sqlquery);
            }
            catch (Exception ex)
            {
        ...
        //void GetListMembers(int ListID,int pageSize, int pageNumber, DateTime startDate,  List<string> fields)
        //{
        //    string EmailDirectAPIKey = System.Configuration.ConfigurationSettings.AppSettings["ed-apikey"];
        //    // ListDetails LD = ListManager.AddNewList(ListName, ListDescription, EmailDirectAPIKey);
        //    PagedResult<Subscriber> PR = ListManager.GetListMembers(21, pageSize, pageNumber, startDate, fields);
        //    foreach (Subscriber s in PR.Items)
        //    {
        //        Console.WriteLine(s.EmailAddress+ " "+s.CustomFields[""].ToString()+ " "+ s.CustomFields[""].ToString());
        //    }            
        //}
        ...
        {
            Console.WriteLine("Start Importing data to Email Direct Petadolex");
