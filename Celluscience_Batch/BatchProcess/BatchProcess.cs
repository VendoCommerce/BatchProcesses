...
{
    public class BatchProcess : ConversionSystems.Celluscience.UI.BaseClass
    {
        static string key1;
        ...
        private string txtPreviousfilename = "";
        const string dq = "\"";
        const string dqcdq = "\",\"";
        ...
        public bool ProcessBatch(DateTime dtbp, string EmployeeNumber)
        {
            bool _breturn = false;
           // string DNIS = "5700";
            string DNIS = Helper.DataServicesTxtBatchConfig["Field#22"];   
            ...
            Previousfilename = EmployeeNumber + DateTime.Now.AddDays(-1).ToString("MMddyyyy") + "_cellu.txt";
            txtfilename = _srtPath + "\\" + Actualfilename ;
            txtPreviousfilename = _srtPath + "\\" + Previousfilename;
            if (File.Exists(txtPreviousfilename))
            {
                File.Delete(txtPreviousfilename);
            }
            StreamWriter SWtxtfile = new StreamWriter(txtfilename);
            SWtxtfile.Flush();
            #endregion

            try
            {  
                DAL.SQLServer.GetOrdersForTxtBatch(dtbp.AddDays(-1), out _dtOrders);
                foreach (DataRow _drOrder in _dtOrders.Rows)
                {
                    ...
                    DAL.SQLServer.GetSalesTaxRate(ord.ShipToState, out _dtSalesTaxRate);
                    ...
                    if (IsMultiPay)
                    {
                        if (ord.ShipToState.Equals("FL"))
                        {
                            ...
                            ord.MerchandiseTotal = Math.Round((decimal.Parse(ord.MerchandiseTotal) * 3), 2).ToString();
                    ...
                    foreach (DataRow _drOrderSKU in OrderSKU.Rows)
                    {
                        ...
                        if (IsMultiPay)
                        {
                            ...
                            PRICE = Math.Round((decimal.Parse(PRICE) * 3), 2).ToString();
                        }
                        else
                        {
                            PRICE = _drOrderSKU["OrderSkuPrice"].ToString();
