...
{
    public class Spain : Com.ConversionSystems.UI.BasePage
    {
        ...
        private DataTable Orders
        {
            get
            {
                   //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.ParseExact (DateTime.Now.ToString("yyyyMMdd"), "yyyyMMdd",CultureInfo.InvariantCulture), out _dtOrders);
                     DAL.SQLServerDALSpain.GetOrdersForXMLBatch(out _dtOrders);
        ...
        private DataTable RejectedOrders
        {
            get
            {
                ...
                {
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.ParseExact (DateTime.Now.ToString("yyyyMMdd"), "yyyyMMdd",CultureInfo.InvariantCulture), out _dtOrders);
                    DAL.SQLServerDALSpain.GetRejectedOrdersForXMLBatch(out _dtRejectedOrders);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.Now.AddDays(-1), out _dtOrders);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(new DateTime(2009, 06, 29), out _dtOrders);
                }
        ...
        private DataTable CustomerBillingAddress
        {
            get
            {
                ...
                {
                    DAL.SQLServerDALSpain.GetCustomerAddress(_intBillingAddress, out _dtCustomerBillingAddress);
                }
        ...
        private DataTable CustomerShippingAddress
        {
            get
            {
                ...
                {
                    DAL.SQLServerDALSpain.GetCustomerAddress(_intShippingAddress, out _dtCustomerShippingAddress);
                }
        ...
        private DataTable OrderSKU
        {
            get
            {
                if (_dtOrderSKU == null)
                {
                    DAL.SQLServerDALSpain.GetOrderSKU(_intOrderID, out _dtOrderSKU);
                }
                return _dtOrderSKU;
            }
        }
        ...
        public void sendxml1(string verifyflag, string ordernumber)
        {
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ...
                s1 = s1.Replace("@orderid@", "ESP" + fixstring(r["orderid"]));
                ...
                foreach (DataRow r1 in OrderSKU.Rows)
                {
                    ...
                    if (r1["skuid"].ToString().Equals("445") || r1["skuid"].ToString().Equals("447"))
                    {
                        stc = stc.Replace("@0", "1");
                    }
                    else
                ...
                if (PaymentPlanSKU.Contains("445") || PaymentPlanSKU.Contains("447"))
                {
       ...
       public void RejectedOrdersCheck()
        ...
        public static void sendDeclineEmail(string DeclineToEmail, string FirstName, string LastName, string version)
        {
            ...
            sb.Append("Please visit our Website (http://www.trynono.com/spain) and place an order with a new card.<br /><br />");
