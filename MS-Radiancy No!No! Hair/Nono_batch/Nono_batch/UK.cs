...
{
    public class UK : Com.ConversionSystems.UI.BasePage
    {
        private DataTable _dtOrders = null;
        private DataTable _dtRejectedOrders = null;
        private DataTable _dtSendEmailConf = null;
        ...
        private DataTable Orders
        {
            get
            {
                ...
                DAL.SQLServerDALUK.GetOrdersForXMLBatch(out _dtOrders);
        ...
        private DataTable RejectedOrders
        {
            get
            {
                ...
                {
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.ParseExact (DateTime.Now.ToString("yyyyMMdd"), "yyyyMMdd",CultureInfo.InvariantCulture), out _dtOrders);
                    DAL.SQLServerDALUK.GetRejectedOrdersForXMLBatch(out _dtRejectedOrders);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.Now.AddDays(-1), out _dtOrders);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(new DateTime(2009, 06, 29), out _dtOrders);
                }
        ...
        private DataTable SendEmailConf
        {
            get
            {
                if (_dtSendEmailConf == null)
                {
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.ParseExact (DateTime.Now.ToString("yyyyMMdd"), "yyyyMMdd",CultureInfo.InvariantCulture), out _dtOrders);
                    DAL.SQLServerDALUK.SendEamilConf(out _dtSendEmailConf);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(DateTime.Now.AddDays(-1), out _dtOrders);
                    //DAL.SQLServerDALUK.GetOrdersForXMLBatch(new DateTime(2009, 06, 29), out _dtOrders);
                }
                return _dtSendEmailConf;
            }
        }
        private DataTable CustomerBillingAddress
        {
            get
            {
                ...
                {
                    DAL.SQLServerDALUK.GetCustomerAddress(_intBillingAddress, out _dtCustomerBillingAddress);
                }
        ...
        private DataTable CustomerShippingAddress
        {
            get
            {
                ...
                {
                    DAL.SQLServerDALUK.GetCustomerAddress(_intShippingAddress, out _dtCustomerShippingAddress);
                }
        ...
        private DataTable OrderSKU
        {
            get
            {
                if (_dtOrderSKU == null)
                {
                    DAL.SQLServerDALUK.GetOrderSKU(_intOrderID, out _dtOrderSKU);
                }
                return _dtOrderSKU;
            }
        }
        ...
        public string getomxml(string verifyflag, string orderid1)
        {
            ...
            s1 += "		    <Field fieldName=[[[WebChat[[[>@WebChatResponse@</Field>~";            
            s1 += "	    </Report>~";
            s1 += "	</CustomFields>~";
            s1 += "</UDOARequest>~";
        ...
        public void sendxml1(string verifyflag, string ordernumber)
        {
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ...
                s1 = s1.Replace("@BuyerEmail@", fixstring(r["email"]));
                if (r["ChatRequest"] != null)
                {
                    if (r["ChatRequest"].ToString().ToLower() == "true")
                        s1 = s1.Replace("@WebChatResponse@", "Yes");
                    else
                        s1 = s1.Replace("@WebChatResponse@", "No");
                }
                else
                ...
                s1 = s1.Replace("@orderid@", "UK" + fixstring(r["orderid"]));
                ...
                s1 = s1.Replace("@1@", fixstring(r["faxnumber"]));
                ...
                foreach (DataRow r1 in OrderSKU.Rows)
                {
                    ...
                    if (r1["skuid"].ToString().Equals("371") || r1["skuid"].ToString().Equals("373") || r1["skuid"].ToString().Equals("406") || r1["skuid"].ToString().Equals("408") || r1["skuid"].ToString().Equals("410") || r1["skuid"].ToString().Equals("412") || r1["skuid"].ToString().Equals("439") || r1["skuid"].ToString().Equals("480") || r1["skuid"].ToString().Equals("482") || r1["skuid"].ToString().Equals("484") || r1["skuid"].ToString().Equals("510") || r1["skuid"].ToString().Equals("512") || r1["skuid"].ToString().Equals("556")
                        || r1["skuid"].ToString().Equals("664")
                        || r1["skuid"].ToString().Equals("666")
                        || r1["skuid"].ToString().Equals("668")

                        || r1["skuid"].ToString().Equals("683")
                        || r1["skuid"].ToString().Equals("685")
                        || r1["skuid"].ToString().Equals("687")

                        || r1["skuid"].ToString().Equals("711")
                        || r1["skuid"].ToString().Equals("750")
                        || r1["skuid"].ToString().Equals("780")
                        || r1["skuid"].ToString().Equals("790")
                ...
                if (PaymentPlanSKU.Contains("405") || PaymentPlanSKU.Contains("406") || PaymentPlanSKU.Contains("407") || PaymentPlanSKU.Contains("408") || PaymentPlanSKU.Contains("409") || PaymentPlanSKU.Contains("410") || PaymentPlanSKU.Contains("411") || PaymentPlanSKU.Contains("412") || PaymentPlanSKU.Contains("479") || PaymentPlanSKU.Contains("481") || PaymentPlanSKU.Contains("483") || PaymentPlanSKU.Contains("509") || PaymentPlanSKU.Contains("510") || PaymentPlanSKU.Contains("511") || PaymentPlanSKU.Contains("512")

                    || PaymentPlanSKU.Contains("682")
                    || PaymentPlanSKU.Contains("683")
                    || PaymentPlanSKU.Contains("684")
                    || PaymentPlanSKU.Contains("685")
                    || PaymentPlanSKU.Contains("686")
                    || PaymentPlanSKU.Contains("687")

                    || PaymentPlanSKU.Contains("796")
                    || PaymentPlanSKU.Contains("797")
                    || PaymentPlanSKU.Contains("798")
                    || PaymentPlanSKU.Contains("799")
                    || PaymentPlanSKU.Contains("800")
                    || PaymentPlanSKU.Contains("801")
                    )
                {
                    cnt++;
                    stc += stb;
                ...
                {
                    if (PaymentPlanSKU.Contains("371") || PaymentPlanSKU.Contains("373") || PaymentPlanSKU.Contains("439") || PaymentPlanSKU.Contains("556")
                        || PaymentPlanSKU.Contains("664")
                        || PaymentPlanSKU.Contains("666")
                        || PaymentPlanSKU.Contains("668")
                        || PaymentPlanSKU.Contains("711")
                        || PaymentPlanSKU.Contains("750")
                        || PaymentPlanSKU.Contains("780")
                        || PaymentPlanSKU.Contains("790")
                    ...
                    else if (PaymentPlanSKU.Contains("405") || PaymentPlanSKU.Contains("406") || PaymentPlanSKU.Contains("407") || PaymentPlanSKU.Contains("408") || PaymentPlanSKU.Contains("409") || PaymentPlanSKU.Contains("410") || PaymentPlanSKU.Contains("411") || PaymentPlanSKU.Contains("412") || PaymentPlanSKU.Contains("509") || PaymentPlanSKU.Contains("510") || PaymentPlanSKU.Contains("511") || PaymentPlanSKU.Contains("512")
                ...
                if (s1.Contains("WEB-ACCUK"))
                {
                    if (PaymentPlanSKU.Contains("480") || PaymentPlanSKU.Contains("482") || PaymentPlanSKU.Contains("484"))
                    {
                        if ((total1z - 64) > 75)
                        {
                            mthCode = "7";
        ...
        public void SendEmailConfirmation()
        {
            ...
            try
            {
                foreach (DataRow _drSendEmailConf in SendEmailConf.Rows)
                {
                    if (_drSendEmailConf["OrderId"].ToString() != null)
                    {

                        try
                        {
                            HttpPost("http://www.trynono.com/UK/Store/Checkout/sendemail.aspx?OrderId=" + _drSendEmailConf["OrderId"].ToString(), "");
                        }
                        catch
                        {
                            Console.WriteLine("Error while sending rejected orders email.");
        ...
        public static void sendDeclineEmail(string DeclineToEmail, string FirstName, string LastName, string version)
        {
            ...
            message.From = new MailAddress("info@trynono.co.uk");
            ...
            sb.Append("Please visit our Website (http://www.trynono.co.uk) and place an order with a new card, or call 0844 571 8287 to place an order by phone.<br /><br />");
        ...
        public static void sendEmailToAdmin(string desc, string orderid)
        {
            try
            {
                ...
                message.Subject = "Error processing order -  Trynono.co.uk" + orderid;
