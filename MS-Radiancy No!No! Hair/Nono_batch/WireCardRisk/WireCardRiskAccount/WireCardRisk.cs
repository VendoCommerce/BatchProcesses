...
namespace EBS.IntegrationServices.Providers.PaymentProviders.WireCardRisk
{
    public class WireCardRisk : GatewayProvider
    {

        #region Private Members

        private static string m_ProviderSection = "PaymentProviderWireCardRisk";
        ...
        public override Response PerformRequest(Request request)
        {
            ...
            request.RequestData["temp_request"] = strPost;
        ...
        protected string BuildRequestPostForCreditCheckScore(Request request)
        {
            ...
            try
            {
                ...
                if (request.AdditionalInfo != null)
                {
                    if (request.AdditionalInfo.ContainsKey("DOB"))
                    {
                        DOB = request.AdditionalInfo["DOB"].ToString();
                    }
                    if (request.AdditionalInfo.ContainsKey("Gender"))
                    {
                        Gender = request.AdditionalInfo["Gender"].ToString();
                    }
                ...
                s1 += "</application>";
                s1 += "<verify>";
                s1 += "<first-name>" + ClearAccents(request.FirstName) + "</first-name>";
                s1 += "<last-name>" + ClearAccents(request.LastName) + "</last-name>";
                s1 += "<gender-code>" + ClearAccents(Gender) + "</gender-code>";
                s1 += "<birth-date>" + DOB + "</birth-date>";
                s1 += "<customer-id>" + request.InvoiceNumber + "</customer-id>";
                s1 += "<current-address>";
                s1 += "<street-name>" + ClearAccents(request.Address1) + "</street-name>";
                s1 += "<street-number>" + ClearAccents(request.Address2) + "</street-number>";
                s1 += "<zip-code>" + ClearAccents(request.ZipCode) + "</zip-code>";
                s1 += "<city>" + ClearAccents(request.City) + "</city>";
                s1 += "<country>" + ClearAccents(request.Country) + "</country>";
                s1 += "</current-address>";
                s1 += "</verify>";
                s1 += "</risk-request>";
        ...
        private Response ParseResponse(string GatewayResponse, Request request)
        {
            ...
            {
                if (request.RequestData.ContainsKey("temp_request"))
                {
                    response.MerchantDefined1 = request.RequestData["temp_request"].ToString();
                }
            }
            ...
            if (request.AdditionalInfo != null)
            {
                if (request.AdditionalInfo.ContainsKey("WireCardRequestType"))
                {
                    SourceCheck = request.AdditionalInfo["WireCardRequestType"].ToString().ToUpper();
                    switch (request.AdditionalInfo["WireCardRequestType"].ToString().ToUpper())
                    {
                        case "DEBIT":
                        ...
                        case "POI":
                            ...
                            break;
                        case "CREDITCHECK":
                            xPath = "risk-response/verifications/credit-risk-verification";
                            break;
                        default:
            ...
            if (xResult != null)
            {
                if (SourceCheck.ToUpper().Equals("CREDITCHECK"))
                {
                    if (xResult["score-class-code"].InnerText.Equals("3"))
                    {
                        response.ResponseType = TransactionResponseType.Approved;
                        response.AuthCode = xResult["score-class-code"].InnerText;
                        response.ReasonText = xResult["return-message"].InnerText;
                    }
                    else
                    {
