...
using FTP_CSVXTRAC.com.hitslink.www;

namespace FTP_CSVXTRAC
{
    class Reports
    {        
        private string targetPath = "C:/BatchFiles/XtracNow/";
        ...
        void LoadDailySales(DateTime ReportDate)
        {
            try
            {
                ...
                DateTime startdate = GetLastHour(DateTime.Now);
                DateTime enddate = GetCurrentHour(DateTime.Now);

                Console.WriteLine("Generating  Daily Lead Reports");                
                string excelFileName = "XLD" + ReportDate.ToString("yyyyMMddmmHH") + "_" + CurrentHour.ToString() + ".txt"; 
                string FUllPAthwithFileName = targetPath + excelFileName;

                DataTable DT = getDataTable("pr_report_MediaPartners", startdate, enddate);
                if (DT.Rows.Count > 0)
                {
                ...
                //uploadFiletoFTP(FUllPAthwithFileName);
                runsql("update [OrderInfo] set status = 2 where status is null and (Created >= '" + startdate + "' ) and (Created < '" + enddate + "')");
                //uploadFile(System.Configuration.ConfigurationSettings.AppSettings["FTPURL"], FUllPAthwithFileName,
                //           System.Configuration.ConfigurationSettings.AppSettings["FTPLogin"],
                //           System.Configuration.ConfigurationSettings.AppSettings["FTPPassword"]);
